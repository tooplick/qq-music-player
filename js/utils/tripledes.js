/**
 * TripleDES implementation for Browser
...
function bitnum(a, b, c) {
    ...
    // JS: index must be integer. 
    // Python: a[(b // 32) * 4 + 3 - (b % 32) // 8]
...
function bitnum_intr(a, b, c) {
    ...
    // Python: (a >> (31 - b)) & 1
    // JS: >>> for logical shift right
    return ((a >>> (31 - b)) & 1) << c;
}

function bitnum_intl(a, b, c) {
    // Python: ((a << b) & 0x80000000) >> c
    // JS: << is fine. & 0x80000000 produces negative if bit 31 set.
    // >>> c to shift right logically (zero fill)
    // But python >> is arithmetic shift?
    // In this algo, 'c' seems to be shift amount.
    // Let's check python usage: bitnum_intl(state, 31, 0)
    // 31 is b. (state << 31) & 0x80000000
    // If state has bit 0 set, << 31 makes it bit 31 (sign bit).
    // & 0x80000000 keeps only sign bit.
    // >> c. 
    // In JS: (a << b) gives int32. 
    // We want UNSIGNED result usually for bit logic.
...
function initial_permutation(input_data) {
    // Returns [int, int] (unsigned 32-bit)
    const r0 = (
...
function key_schedule(key, mode) {
    ...
    let c = 0;
    // Python: sum(bitnum(key, key_perm_c[i], 31 - i) for i in range(28))
    ...
    for (let i = 0; i < 16; i++) {
        // Python: c = ((c << key_rnd_shift[i]) | (c >> (28 - key_rnd_shift[i]))) & 0xFFFFFFF0
        // JS: >>>
...
// Global Pako/DecompressionStream wrapper
// Prefer Pako if available (loaded from CDN in index.html)
// Fallback to DecompressionStream
async function decompress(data) {
    // Check global pako
    if (typeof window.pako !== 'undefined') {
        try {
            return window.pako.inflate(data, { to: 'string' });
        } catch (e) {
            // console.warn('Pako inflate failed, trying raw');/
            try {
                return window.pako.inflateRaw(data, { to: 'string' });
            } catch (e2) {
                console.error('Pako failed:', e2);
            }
        }
    }

    // Fallback to DecompressionStream (Worker/Modern Browser)
    ...
    try {
        ...
        if (data.length > 2) {
            try { return await tryDecompress(data.slice(2)); } catch (e2) { }
        }
        return '';
    }
}
...
export async function qrc_decrypt(encrypted_qrc_hex) {
    ...
    // Verify self test
    // Expected: [1, 94, 166, 156, 35, 48, 60, 238]
    // Input: 12345678 (ascii) -> [49, 50, 51, 52, 53, 54, 55, 56]
    // Key: !@#)(*$%123ZXC!@!@#)(NHL
    try {
        const testKey = new TextEncoder().encode("!@#)(*$%123ZXC!@!@#)(NHL");
        const testData = new TextEncoder().encode("12345678");
        const testSch = tripledes_key_setup(testKey, ENCRYPT); // Wait, python test used ENCRYPT=1

        // Wait, Python test: tripledes_crypt(bytearray(data), tripledes_key_setup(key, 1))
        // 1 is ENCRYPT (in my JS consts)
        const testEnc = tripledes_crypt(testData, testSch);

        // console.log('Self Test Result:', testEnc);
        // [1, 94, 166, 156, 35, 48, 60, 238]
        // Check first byte
        if (testEnc[0] !== 1 || testEnc[7] !== 238) {
            console.error("FATAL: TripleDES JS implementation does not match Python!", testEnc);
        }
    } catch (e) {
